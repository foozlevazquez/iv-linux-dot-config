...
function toJSON(o)
{
  ...
  function subObjToJSON(o,p,ref)
  {
    ...
    else
    {
      ...
      // tkaraszewski.
      // The first clause in this 'or' works for arrays that were created in thw same window as the one we're working
      // in, but it fails if we pass an array from a different javascript context. The second clause seems to handle
      // that, at least in the cases we've seen.
      else if ((o.constructor === Array) || (Object.prototype.toString.call(o) === '[object Array]'))
      {
...
// tkaraszewski: changed max_request_active from 1 to 2 to avoid unnessecary pipelining of requests, otherwise
// slow/long-lived requests can block outstanding small/fast requests.
...
JSONRpcClient._sendRequest = function (client,req)
{
  ...
  try
  {
    http.setRequestHeader("X-Evernote-Agent", Browser.getAgent());
  }
  catch(e)
  {
  }
...
JSONRpcClient.prototype.unmarshallResponse=function(data)
{
  ...
  try
  {
    // tkaraszewski: eval doesn't work in chrome extension manifest v2.
    obj = JSON.parse(data);
    // eval("obj = " + data);
  }
  catch(e)
  {
    throw new JSONRpcClient.Exception({ code: 550, message: "error parsing result", data: data });
  } 
  if (obj.error)
  {
