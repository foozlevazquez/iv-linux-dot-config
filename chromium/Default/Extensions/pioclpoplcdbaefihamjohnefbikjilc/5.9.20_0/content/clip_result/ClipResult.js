...
function msgHandlerConfig(request, sender, sendResponse) {
  ...
  relatedSnippets = new NoteSnippets(document.querySelector("#relatedNotesContainer"), baseUrl, auth.userId,
    auth.shardId, options.client, { pers: auth.authenticationToken, biz: auth.bizAuthenticationToken });
...
function doSharedNoteSuccessAction(action, evt) {
  if (action == "view") {
    var url = GlobalUtils.getNoteURI(options.client, baseUrl, message, action,
      auth.userId, 0, { pers: auth.authenticationToken, biz: auth.bizAuthenticationToken });
