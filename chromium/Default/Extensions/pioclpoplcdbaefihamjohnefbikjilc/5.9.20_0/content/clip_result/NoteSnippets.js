function NoteSnippets(attachmentPoint, baseUrl, userId, shardId, client, tokens) {
  "use strict";
  ...
  function renderBlock(note, offset) {
    ...
    if (note.thumbsquare) {
      ...
      img.src = baseUrl + "/SetAuthToken.action?auth="
        + encodeURIComponent(tokens.pers) + "&targetUrl="
        + encodeURIComponent(baseUrl + note.thumbsquare + "?size=" + size);
    }
