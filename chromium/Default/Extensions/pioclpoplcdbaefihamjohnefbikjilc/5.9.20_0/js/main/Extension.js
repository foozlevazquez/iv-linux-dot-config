function Extension() {
  ...
  window.addEventListener("offline", function(e) {console.log("Went offline.");});
  ...
  function setDeviceID() {
    if (!Persistent.get("deviceID")) {
      Persistent.set("deviceID", UUID.generateGuid().slice(0, 32));
    }
  }

  function purgeLocalStorage() {
    if (!Persistent.get("localStoragePurged")) {
      Persistent.set("localStoragePurged", true);
      delete localStorage.savedAuthInfo;
  ...
  function contextMenuActionFailed() {
    if (OPERA) {
      chrome.tabs.getSelected(function(t) {
        chrome.tabs.executeScript(t.id, { code: "alert(Browser.i18n.getMessage(\"contextMenuPleaseLogin\"))" });
      });
    } else {
  ...
  function contextMenuClipPage(info, tab) {
    ...
    auth.isAuthenticated(checkAuthCallback);
  }
  ...
  function contextMenuNewNote(info, tab) {
    ...
    function checkAuthCallback(auth) {
      if (auth) {
        var url = options.get("secureProto") + bootstrapInfo.get("serviceHost")
          + "/SetAuthToken.action?auth=" + encodeURIComponent(auth.authenticationToken)
          + "&targetUrl=" + encodeURIComponent("/edit?newNote");
        openTab(url);
      } else {
  ...
  function setBadge() {
      ...
      for (var i in pendingNotes) {
        ...
        if (pendingNotes[i] && pendingNotes[i].status) {
          ...
          console.warn("Note was null or had no status, skipping and deleting. " + JSON.stringify(pendingNotes[i]));
          delete pendingNotes[i];
        }
      }
  ...
  function JsonRpcInit(callback) {
    try {
      ...
      auth.getAuthTokens(function(tokens) {
  ...
  function clipEmailFromTab(tab, note) {
    if (!note) {
      var notebooks = resolveNotebook();
      note = new PendingNote({}, notebooks, tab);
    }
    Browser.sendToTab(tab, {name: "clipEmail", note: note,
      notify: options.get("clipNotificationEnabled")});
  }
  ...
  Browser.addMessageHandlers({
    ...
    downloadAttachments: msgHandlerDownloadAttachments,
    ...
    getPersistentFlag: msgHandlerGetPersistentFlag
  });
  ...
  function msgHandlerIsAuthenticated(request, sender, sendResponse) {
    function sendMsg(msg) {
      if (request.type == "simSearch") {
        ...
        var reply = { name: "tooltip_isAuthenticated", auth: msg };
        if (request.bootstrapInfo) {
          reply.bootstrapInfo = {};
          for (var key in request.bootstrapInfo) {
            reply.bootstrapInfo[key] = bootstrapInfo.get(key);
          }
        }
        Browser.sendToTab(sender.tab, reply);
      }
    }
    if (sender && sender.tab) {
      auth.isAuthenticated(sendMsg);
    }
    else {
      auth.isAuthenticated(sendResponse);
    }
  }
  ...
  function msgHandlerLogin(request, sender, sendResponse) {
    ...
    auth.login(request.data.username, request.data.password, sendResponse);
  }
  ...
  function msgHandlerLogout(request, sender, sendResponse) {
    ...
    if (request.all) {
      ...
      auth.logout(sendResponse, request.authExpired);
    }
  }
  ...
  function msgHandlerClipEmail(request, sender, sendResponse) {
    clipEmailFromTab(request.tab, request.attrs);
    clearAllPreviews();
    if (sendResponse) sendResponse();
  }

  function executeAllLinkedNotebookCallbacks(response) {
    ...
    for (var i = 0; i < linkedNotebookCallbacks.length; i++) {
      ...
      try {
        ...
        console.debug("Linked Notebook callback exception: ", e.message, e.stack);
      }
    }
  ...
  function msgHandlerGetWritableLinkedNotebooks(request, sender, sendResponse) {
    ...
    var currentUser = "";
    if (auth) {
      currentUser = auth.getUserInfo().username;
    }
    if (!currentUser) {
    ...
    auth.getAuthTokens(function(tokens) {
      ...
      var authInfo = { token: tokens.authenticationToken };
      if (tokens.bizAuthenticationToken) {
        authInfo.businessId = userInfo.businessId;
      }
      linkedNotebooks = new LinkedNotebooks(authInfo, mainJsonRpc,
        executeAllLinkedNotebookCallbacks);
    });
  }
  ...
  function msgHandlerRequestBizNotebooks(request, sender, sendResponse) {
    function receiveNotebooks(response, exception) {
      if (response) {
        var newResponse = { list: [] };
        for (var i = 0; i < response.list.length; i++) {
          if (linkedNotebooks.getBusinessNotebookByGuid(response.list[i].guid)) {
            newResponse.list.push(response.list[i]);
          }
        }
        bizNotebooks = new BusinessNotebooks(request.auth, newResponse.list);
        sendResponse(newResponse);
      } else {
        console.debug("problem retrieving business notebooks: ", exception);
      }
    }
  ...
  function msgHandlerPageClipSuccess(request, sender, sendResponse) {
    ...
    if (request.note.attachments) {
      var temp = request.note.attachments;
      request.note.attachments = {};
      for (var i = 0; i < temp.length; i++) {
        if (downloadedAttachments[temp[i]]) {
          request.note.attachments[temp[i]] = downloadedAttachments[temp[i]];
          noteSize += request.note.attachments[temp[i]].binary.byteLength;
        }
      }
    }
  ...
  function msgHandlerDownloadAttachments(request, sender, sendResponse) {
    for (var i = 0; i < request.attachments.length; i++) {
      var xhr = new XMLHttpRequest();
      xhr.open("GET", request.attachments[i].url);
      xhr.url = request.attachments[i].url;
      xhr.name = request.attachments[i].name;
      xhr.responseType = "arraybuffer";
      xhr.onreadystatechange = function() {
        if (this.readyState == 4) {
          if (this.status == 200) {
            downloadedAttachments[this.url] = {
              mime: this.getResponseHeader("Content-Type"),
              binary: this.response,
              hexHash: SparkMD5.ArrayBuffer.hash(this.response),
            };
            if (this.name) {
              downloadedAttachments[this.url].name = this.name;
            }
            Browser.sendToTab(sender.tab, { name: "finishAttachmentDownload", url: this.url, success: true });
          } else {
            delete downloadedAttachments[this.url];
            Browser.sendToTab(sender.tab, { name: "finishAttachmentDownload", url: this.url, success: false });
          }
        }
      };
      xhr.send();
  ...
  function msgHandlerReceivePageInfo(request, sender, sendResponse) {
    ...
    function sendRelatedNotes() {
      ...
      if (request.info.searchEngine === "Google" || !authInfo.bizAuthenticationToken) {
        ...
        Browser.sendToTab(sender.tab, {name: "simsearch_receiveRelatedNotes",
          relatedNotes: filteredNotes[0], type: "account",
          isBizUser: Boolean(authInfo.bizAuthenticationToken),
          tokens: { pers: authInfo.authenticationToken, biz: authInfo.bizAuthenticationToken } });
        if (authInfo.bizAuthenticationToken) {
          Browser.sendToTab(sender.tab, {name: "simsearch_receiveRelatedNotes",
            relatedNotes: filteredNotes[1], type: "library",
            tokens: { pers: authInfo.authenticationToken, biz: authInfo.bizAuthenticationToken } });
        }
      }
      else {
        ...
        Browser.sendToTab(sender.tab, {name: "simsearch_receiveRelatedNotes", relatedNotes: filteredNotes,
          type: "all", tokens: { pers: authInfo.authenticationToken, biz: authInfo.bizAuthenticationToken } });
      }
    }
    ...
    */
    ...
    function receivePersonalSmartFilingInfo(response, exception) {
      if (response) {
        ...
        resolveError(exception, sender.tab);
      }
    }
      ...
      for (var i = 0; i < bizRelatedNotes.length; i++) {
        if (containingNotebooks[bizRelatedNotes[i].notebookGuid].joined) {
          if (linkedNotebooks.getBusinessNotebookByGuid(bizRelatedNotes[i].notebookGuid) == null) {
            var authStuff = { token: authInfo.authenticationToken };
            if (authInfo.businessId) {
              authStuff.businessId = authInfo.businessId;
            }
            linkedNotebooks = new LinkedNotebooks(authStuff, rpc, receiveLinkedNotebooks, sender.tab);
            return false;
          }
        }
      }
    ...
    function getRelatedPersonalNotes() {
      JsonQueue.handleExpensiveOpRequest({ shardId: rpc.shardId, userId: authInfo.userId,
        type: "pers" }, rpc.client.NoteStoreExtra.getFilingRecommendations,
        receivePersonalSmartFilingInfo, authInfo.authenticationToken,
        getFilingRecommendationRequest());
    }

    function getRelatedBizNotes() {
      JsonQueue.handleExpensiveOpRequest({ shardId: bizRpc.shardId,
        userId: authInfo.userId, type: "biz"}, bizRpc.client.NoteStoreExtra.getFilingRecommendations,
    ...
    auth.isAuthenticated(requestNotes);
  }
  ...
  function msgHandlerPerformNoteSearch(request, sender, sendResponse) {
    ...
    function performSearch() {
      rpc.client.NoteStoreExtra.findNotesWithSnippet(receivePersonalNotes,
        authInfo.authenticationToken, noteFilter, 0, maxNotes, resultSpec);
      if (authInfo.bizAuthenticationToken) {
        bizRpc.client.NoteStoreExtra.findNotesWithSnippet(receiveBizNotes,
          authInfo.bizAuthenticationToken, noteFilter, 0, maxNotes, resultSpec);
      }
    }
  ...
  function msgHandlerIncrementUser(request, sender, sendResponse) {
    if (!auth) return;
    var users = auth.getLoggedInUserIds();
    if (users.length <= 1) {
      return;
    }
    ...
    Browser.sendToTab(sender.tab, {name: "content_userChanged", username: auth.getUserInfo(users[next]).username});
  }
  ...
  function getLoggedInUserIds() {
    if (!auth) return [];
    return auth.getLoggedInUserIds();
  }
  ...
  function msgHandlerGetPersistentFlag(request, sender, sendResponse) {
    Browser.sendToTab(sender.tab, {
      name: "receivePersistentFlag",
      key: request.key,
      value: Persistent.get(request.key)
    });
    if (request.set) {
      Persistent.set(request.key, request.setValue);
    }
  }

  function msgHandlerSubmitTwoStepCode(request, sender, sendResponse) {
    auth.completeTwoFactorAuthentication(request.auth, request.code, sendResponse);
  }

  function handlePopupExpensiveOpRequest() {
    JsonQueue.handleExpensiveOpRequest.apply(this, arguments);
  }

  function recordCountlyEvent(name, seg) {
    Countly.recordEvent(name, seg);
  }
  ...
  this.msgHandlerClipEmail = msgHandlerClipEmail;
  ...
  this.msgHandlerSubmitTwoStepCode = msgHandlerSubmitTwoStepCode;
  this.handlePopupExpensiveOpRequest = handlePopupExpensiveOpRequest;
  this.recordCountlyEvent = recordCountlyEvent;
