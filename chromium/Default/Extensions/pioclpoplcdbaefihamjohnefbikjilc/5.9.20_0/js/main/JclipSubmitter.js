function JclipSubmitter(authObject, pendingNote){
  ...
  var username = authObject.getUserInfo().username;
  ...
  authObject.getAuthTokens(receiveAuthTokens);
  ...
  function submit() {
    ...
    if (pendingNote.relatedNotes || linked) {
      ...
      submitter.client.NoteStoreExtra.clipNote(handleSubmissionResponse,
        ...
        submitter.client.NoteStoreExtra.clipNoteAndFindSimilar(handleSubmissionResponse,
          submitAuth, pendingNote.noteFiling, pendingNote.title, pendingNote.url,
          pendingNote.content, similarNotesRequest);
  ...
  function handleSubmissionResponse(response, exception) {
    if (exception) {
      ...
      if (!Persistent.get(authObject.getCurrentUser() + "_foodPromoShown")) {
      ...
      if (pendingNote.relatedNotes) {
        handleComplete(submissionResponse, null);
      } else if (personalRelatedNotesResponse
  ...
  function applicationLayerErrorHandler(exception) {
    ...
    if (exception && exception.trace) {
      ...
      } else if (exception.trace.match(/EDAMUserException/) && exception.trace.match(/AUTH_EXPIRED/) && /parameter:authenticationToken/.test(exception.trace)) {
        exception.msg = "authExpired";
        Browser.extension.getBackgroundPage().extension.msgHandlerLogout({});
      }
    }
      ...
      else if (exception.errorCode == 9 && exception.parameter == "authenticationToken") {
        exception.msg = "authExpired";
        Browser.extension.getBackgroundPage().extension.msgHandlerLogout({});
      }
    }
  ...
  function getPersonalRelatedNotes() {
    JsonQueue.handleExpensiveOpRequest({ shardId: shardId,
      userId: authObject.getCurrentUser(), type: "pers" },
  ...
  function getBizRelatedNotes() {
    JsonQueue.handleExpensiveOpRequest({ shardId: bizShardId,
      userId: authObject.getCurrentUser(), type: "biz" },
  ...
  function handleRecipeClassification(recipeClassification) {
    function receiveFoodNotes(response, exception) {
      if (response) {
        Persistent.set(authObject.getCurrentUser() + "_foodPromoShown", true);
