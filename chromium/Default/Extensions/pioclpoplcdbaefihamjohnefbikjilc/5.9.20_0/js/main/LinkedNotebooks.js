...
function LinkedNotebooks(authStuff, jsonRpc, callback, tab) {
  "use strict";
  ...
  var authToken = authStuff.token;

  function receiveNotebookList(data, exception) {
    ...
    // If there aren't any, we still need to fire our callback.
      ...
      if (exception) {
        if (/AUTH_EXPIRED/.test(exception.trace) && /parameter:authenticationToken/.test(exception.trace)) {
          if (tab) {
    ...
    if (auth && data) {
      ...
      for (var i = 0; i < notebookList.length; i++) {
        if (data.shareKey && notebookList[i].shareKey === data.shareKey) {
          ...
          // don't include the business notebooks that are in your business vault
          if (notebookList[i].businessId != authStuff.businessId) {
