...
function Popup() {
  ...
  var smartFilingNotebook;
  var smartFilingNotebookType;
  ...
  var specialLoginErrorMessageMap = {
    ...
    "INVALID_TWO_STEP_CODE": ["incorrectCode"],
    "EXPIRED_AUTHENTICATION_TOKEN": ["expiredTwoStepCode"]
  };
  ...
  function loginReset() {
    ...
    smartFilingNotebook = null;
    smartFilingNotebookType = null;
  ...
  document.querySelector("#twoStepView .continue").addEventListener("click", submitTwoStep);
  document.querySelector("#twoStepView .cancel").addEventListener("click", cancelTwoStep);
  document.querySelector("#twoStepView input").addEventListener("keydown", function() { clearAllErrors(); });
  document.querySelector("#twoStepView .helpLink").addEventListener("click", openTwoStepHelp);
  ...
  */
  ...
  function checkSubmit(evt) {
    ...
    } else if (!/view/.test(document.querySelector("#twoStepView").className)) {
      if (evt.keyCode == 13) {
        submitTwoStep();
      }
    }
  ...
  function submitNote() {
    var note = buildNote();
    if (note.notebookGuid && smartFilingNotebook) {
      var compareNb;
      var preferredNotebooks = Persistent.get("recentNotebooks");
      if (preferredNotebooks && preferredNotebooks[username]) {
        compareNb = preferredNotebooks[username];
      } else {
        compareNb = defaultNB;
      }
      var chosenNotebookType = null;
      var guidParts = note.notebookGuid.split("_");
      if (guidParts.length == 2) {
        if (guidParts[0] == "biz") {
          chosenNotebookType = "biz";
        }
      }
      if (!chosenNotebookType) {
        chosenNotebookType = "pers";
      }
      var chosenNotebook = guidParts[guidParts.length - 1];
      if (compareNb != smartFilingNotebook) {
        var seg = {
          suggested_notebook_type: smartFilingNotebookType
        };
        if (compareNb == chosenNotebook) {
          seg["changed_to_dumb_notebook"] = true;
        } else {
          seg["changed_to_dumb_notebook"] = false;
        }
        if (chosenNotebook == smartFilingNotebook) {
          seg["final_action"] = "kept_suggestion";
        } else if (chosenNotebookType == "pers") {
          seg["final_action"] = "switched_to_personal_notebook";
        } else if (chosenNotebookType == "biz") {
          seg["final_action"] = "switched_to_business_notebook";
        }
        background.recordCountlyEvent("submit_note", seg);
      }
    }
    ...
    switch(clipType) {
      ...
      case "CLIP_ACTION_PDF":
        ...
        break;
      case "CLIP_ACTION_EMAIL":
        background.msgHandlerClipEmail(msg, null, dismissPopup);
        break;
    }
  }
  ...
  function openWebClient() {
    ...
    if (authenticationToken) {
      url = background.getOption("secureProto") + bootstrapInfo.get("serviceHost")
        + "/SetAuthToken.action?auth=" + encodeURIComponent(authenticationToken) + "&targetUrl="
  ...
  function getInactiveFreeUser() {
    for (var i = 0; i < loggedInUsers.length; i++) {
      if (userType(loggedInUsers[i]) == "FREE" && loggedInUsers[i] != userId) {
  ...
  function keepOldFree() {
    ...
    background.msgHandlerLogout({}, null, callback);
  }
  ...
  function keepNewFree() {
    ...
    background.msgHandlerLogout({ user: getInactiveFreeUser() }, null, callback);
  }
  ...
  function showSwitchFree() {
    ...
    document.querySelector("#freeAccountLimit").innerHTML = 
      Browser.i18n.getMessage("freeAccountLimit", background.getUserInfo(getInactiveFreeUser()).username);
  ...
  function handleLoginSuccess() {
    loggedInUsers = background.getLoggedInUserIds();
  ...
  function responseHandlerLogin(response) {
    ...
    if (!response.error) {
      ...
      if (response.secondFactorDeliveryHint) {
        expiration = response.expiration;
        showTwoStep(response.secondFactorDeliveryHint);
        ...
        if (response.bizAuthenticationToken) {
          ...
          if (Persistent.get(response.username + "_turnedOffSearchHelper")) {
            Persistent.set(response.userId + "_turnedOffSearchHelper", true);
            delete localStorage[response.username + "_turnedOffSearchHelper"];
          }
          if (!Persistent.get(response.userId + "_turnedOffSearchHelper")) {
      ...
      if (specialLoginErrorMessageMap[error]) {
        ...
        } else if (error.match(/TWO_STEP_CODE/)) {
          setError("twoStepCode", specialLoginErrorMessageMap[error][0]);
      ...
      if (loginErrorMessageMap[error]) {
        ...
        console.debug("Got error " + error + ", but no message to show.");
      }
  ...
  function doLogout(all, authExpired) {
    hideView("waiting");
    hideView("clip");
    hideView("login");
    var msg = {};
    ...
    if (authExpired) {
      msg.authExpired = true;
    }
    background.msgHandlerLogout(msg);
    if ((loggedInUsers && loggedInUsers.length > 1 && !all) || authExpired) {
      reset();
      init();
    }
    else {
  ...
  function switchUser(evt) {
    var user = "";
    if (this && this.dataset && this.dataset.userId) {
  ...
  function getUserNameList() {
    ...
    for (var i = 0; i < loggedInUsers.length; i++) {
      ...
      li.innerText = Browser.i18n.getMessage("switchToAccount", [background.getUserInfo(loggedInUsers[i]).username]);
      li.dataset.userId = loggedInUsers[i];
      if (mac) {
        li.title = "Ctrl + Cmd + A";
      }
      else {
  ...
  function userType(userId) {
    var userInfo = background.getUserInfo(userId);
    if (userInfo) {
      if (userInfo.bizAuthenticationToken) {
        return "BUSINESS";
  ...
  function showClip() {
    ...
    if (tab.title && (tab.title != tab.url)) {
      if (pageInfo.gmailThread && /^(.+) - (.+) - (.+)$/.test(tab.title)) {
        tab.title = /^(.+) - (.+) - (.+)$/.exec(tab.title)[1];
      }
  ...
  function populateBizNotebooks() {
    ...
    if (cache && cache[username] && cache[username]["bizNotebooks"]) {
      // hack to fix a mistake in 5.9.16 where I made stored a corrupted
      // notebook list. clear the cached version in that case.
      var needToDelete = false;
      for (var i = 0; i < cache[username]["bizNotebooks"].list.length; i++) {
        if (!cache[username]["bizNotebooks"].list[i]) {
          needToDelete = true;
          break;
        }
      }
      if (needToDelete) {
        delete cache[username]["bizNotebooks"];
    ...
    background.msgHandlerRequestBizNotebooks({rpc: bizRpc, auth: bizAuthenticationToken}, null, populateBizNotebooksFromNetwork);
  }
  ...
  function populateTags() {
    ...
    function populateTagsFromNetworkWithUsername(data, error) {
      if (error) {
        resolveError(requestingUser, error);
      } else {
        populateTagsFromNetwork(requestingUser, data);
      }
    }
  ...
  function populateTagsFromNetwork(fromUser, data) {
    if (data && data.list) {
      var tags = [];
  ...
  function populateBizTags() {
    ...
    function populateTagsFromNetworkWithUsername(data, error) {
      if (error) {
        ...
        populateBizTagsFromNetwork(requestingUser, data);
      }
    }
    if (!bizTagsRequested) {
  ...
  function populateBizTagsFromNetwork(fromUser, data) {
    if (data && data.list) {
      var tags = [];
      for (var i = 0; i < data.list.length; i++) {
        if (data.list[i].name) {
  ...
  function setEnabledPreviewTypes() {
    ...
    if (opt) {
      ...
      if (pageInfo.pdf || pageInfo.gmailThread) opt.disabled = true;
    }
    ...
    if (opt) {
      opt.disabled = false;
      if (pageInfo.gmailThread) opt.disabled = true;
    }
    ...
    opt = document.querySelector("select#clipAction option[value=CLIP_ACTION_EMAIL]");
    if (opt) {
      if (pageInfo.gmailThread) opt.disabled = false;
      else opt.disabled = true;
    }
  }
  ...
  function populateNotebooks() {
    ...
    var requestingUser = username;
    function populateNotebooksFromNetworkWithUsername(response, exception) {
      if (response) {
        populateNotebooksFromNetwork(requestingUser, response);
      } else {
        resolveError(requestingUser, exception);
      }
    }
  ...
  function getSmartFilingInfo() {
    var requestingUser = username;
    function receivePersSmartFilingInfoWithUsername(response, exception) {
      receivePersSmartFilingInfo(requestingUser, response, exception);
    }
    function receiveBizSmartFilingInfoWithUsername(response, exception) {
      receiveBizSmartFilingInfo(requestingUser, response, exception);
    }
    ...
    if (!setting != "NONE") {
      ...
      if (pageInfo.recommendationText) {
        var filingRecommendationRequest = {text: pageInfo.recommendationText,
          query: pageInfo.query, url: tab.url, relatedNotesResultSpec: resultSpec };
        background.handlePopupExpensiveOpRequest({ shardId: jsonRpc.shardId, userId: userId, type: "pers" },
          jsonRpc.client.NoteStoreExtra.getFilingRecommendations, receivePersSmartFilingInfoWithUsername,
          authenticationToken, filingRecommendationRequest);
        if (bizAuthenticationToken) {
          filingRecommendationRequest.relatedNotesResultSpec.maxResults = 10;
          background.handlePopupExpensiveOpRequest({ shardId: bizRpc.shardId,
            userId: userId, type: "biz" },
            bizRpc.client.NoteStoreExtra.getFilingRecommendations,
            receiveBizSmartFilingInfoWithUsername, bizAuthenticationToken,
  ...
  function msgHandlerReceivePageInfo(response) {
    ...
    if (pageInfo.gmail && !pageInfo.gmailThread) {
      showGlobalError("gmailError");
      return;
    }
  ...
  function pickClipType() {
    ...
    if (pageInfo) {
      ...
      else if (pageInfo.gmailThread) {
        type = "CLIP_ACTION_EMAIL";
      }
  ...
  function receivePersSmartFilingInfo(requestingUser, data, exception) {
    if (data) {
  ...
  function receiveBizSmartFilingInfo(requestingUser, data, exception) {
    if (data) {
      ...
      if (bizSmartFilingInfo.notebook) {
        bizSmartFilingInfo.notebook.biz = true;
      }
  ...
  function selectSmartFilingNotebook(data) {
    ...
    if ((background.getOption("smartFilingEnabled") == "ALL") ||
      ...
      if (data.notebook && !pageInfo.gmailThread) {
        var select = document.querySelector("#notebookControl");
        var nbguid = data.notebook.guid;
        if (data.notebook.biz) {
          nbguid = "biz_" + nbguid;
        }
        for (var i = 0; i < select.options.length; i++) {
  ...
  function populateSmartFilingInfo(data) {
    ...
    if (((background.getOption("smartFilingEnabled") == "ALL") ||
      (background.getOption("smartFilingEnabled") == "NOTEBOOKS")) &&
        data.notebook && !pageInfo.gmailThread) {
      smartFilingNotebook = data.notebook.guid;
      smartFilingNotebookType = data.notebook.biz ? "biz" : "pers";
    ...
    if (!userSelectedTags) {
      if ((background.getOption("smartFilingEnabled") == "ALL") ||
        (background.getOption("smartFilingEnabled") == "TAGS") && !pageInfo.gmailThread) {
  ...
  function responseHandlerCheckLogin(response) {
    ...
    if (response) {
      ...
      if (response.bizAuthenticationToken) {
        bizAuthenticationToken = response.bizAuthenticationToken;
        bizExpiration = response.bizExpiration;
      }
  ...
  function checkPageReady() {
    ...
    Browser.sendToTab(tab, {name: "content_ready_5_9_20"});
  ...
  function init() {
    ...
    if (!SAFARI) {
      try {
        ...
        console.log("Couldn't connect to extension: ", e);
      }
    }
  ...
  function resolveError(requestingUser, error) {
    if (username == requestingUser) {
      if ((/PERMISSION_DENIED/.test(error.trace) || /AUTH_EXPIRED/.test(error.trace)) && /parameter:authenticationToken/.test(error.trace)) {
        doLogout(false, true);
      } else {
  ...
  function showTwoStep(phone) {
    hideView("waiting");
    hideView("login");
    hideView("registration");
    hideView("switchFree");
    hideView("clip");
    document.querySelector("#twoStepView").className = "";
    document.querySelector("#twoStepView input").placeholder = Browser.i18n.getMessage("sixDigitCode");
    document.querySelector("#twoStepView input").value = "";
    document.querySelector("#twoStepView input").focus();
    document.querySelector("#twoStepView .twoStepIcon").className = document.querySelector("#twoStepView .twoStepIcon").className.replace(/\s*sms\s*/, "").replace(/\s*ga\s*/, "");
    if (phone) {
      document.querySelector("#twoStepView .instruction").innerText = Browser.i18n.getMessage("smsMessage", [phone]);
      document.querySelector("#twoStepView .twoStepIcon").className += " sms";
    } else {
      document.querySelector("#twoStepView .instruction").innerText = Browser.i18n.getMessage("gaMessage");
      document.querySelector("#twoStepView .twoStepIcon").className += " ga";
    }
  }

  function submitTwoStep() {
    clearError("twoStepCode");
    var code = document.querySelector("#twoStepView input");
    code = code.value.replace(/-/g, "");
    if (/^(\d{6}|\d{16})$/.test(code)) {
      background.msgHandlerSubmitTwoStepCode({ auth: authenticationToken, code: code },
        null, responseHandlerLogin);
    } else {
      setError("twoStepCode", "incorrectCode");
    }
  }
  ...
  function openTwoStepHelp() {
    if (expiration > new Date()) {
      var url = background.getOption("secureProto") + bootstrapInfo.get("serviceHost")
        + "/TwoStepHelp.action?auth=" + encodeURIComponent(authenticationToken);
      background.msgHandlerOpenTab({name: "main_openTab", url: url}, null, dismissPopup);
    } else {
      responseHandlerLogin({ error: "EXPIRED_AUTHENTICATION_TOKEN" });
    }
  }
...
// Run in popup.html.
window.addEventListener("error", function(e) {console.debug(JSON.stringify(e), e);});
if (SAFARI) {
  var timeout;
