...
(function(){
  ...
  GlobalUtils.createUrlClipContent = function(title, url, favIcoUrl) {
    if (/^https?:\/\/docs\.google\.com\/viewer\?url=.+/.test(url)) {
      url = decodeURIComponent(/^https?:\/\/docs\.google\.com\/viewer\?url=(.+)/.exec(url)[1]);
    }
  ...
  GlobalUtils.getNoteURI = function(client, baseUrl, message, action, userId, currentUSN, tokens) {
    var uri;
    if (client == "WEB") {
      if (message.shareKey) { // shared notebook
        uri = "/shard/" + message.shardId + "/share/view/" + message.shareKey + "/#n=" + message.noteGuid;
      }
      else if (message.inBusinessNotebook) {
        uri = "/shard/" + message.shardId + "/business/dispatch/" + action + "/"
          + message.noteGuid + "#st=b" + "&n=" + message.noteGuid;
        if (message.notebookGuid) {
          uri += "&b=" + message.notebookGuid;
        }
      }
      else {
        uri = "/" + action + "/" + message.noteGuid + "#st=p";
      }
      uri = baseUrl + "/SetAuthToken.action?auth=" + encodeURIComponent(tokens.pers) + "&targetUrl=" + encodeURIComponent(uri);
    }
  ...
  GlobalUtils.setupCache = function(obj, maxKeys, maxAge) {
    return function() {
      var timeouts = {};
      var cache = {};

      obj.get = function(userId, type, key) {
        if (cache[userId] && cache[userId][key] && cache[userId][key][type]) {
          if (timeouts[userId][key]) {
            clearTimeout(timeouts[userId][key]);
            timeouts[userId][key] = setTimeout(function() {
              delete cache[userId][key];
            }, maxAge);
          }
          return cache[userId][key][type];
        }
        return null;
      };

      obj.add = function(userId, type, key, content) {
        if (!cache[userId]) {
          cache[userId] = {};
        }
        // remove least recently used if we've hit the limit
        if (Object.keys(cache[userId]).length > maxKeys) {
          var min = 2147483647;
          var minKey;
          for (var i in timeouts[userId]) {
            if (timeouts[userId][i] < min) {
              min = timeouts[userId][i];
              minKey = i;
            }
          }
          clearTimeout(min);
          delete timeouts[userId][minKey];
          delete cache[userId][minKey];
        }

        if (!cache[userId][key]) {
          cache[userId][key] = {};
        }
        cache[userId][key][type] = content;
